...
class Data:
    ...
    def __init__(self, args):
        ...
        self.semi_input_ids, self.semi_input_mask, \
        self.semi_segment_ids, self.semi_label_ids, \
        self.ind_semi, self.semi_label_real = self.get_semi(
        ...
        self.train_semi_dataset, self.train_semi_dataloader = self.get_semi_loader(
            self.semi_input_ids, self.semi_input_mask,
            self.semi_segment_ids, self.semi_label_ids,
            self.ind_semi, self.semi_label_real, args)
    ...
    def get_semi(self, labeled_examples, unlabeled_examples, args):
        ...
        if self.n_known_cls > 0:
            ...
            size_semi += len(labeled_examples)
        else:
        ...
        size_semi += len(unlabeled_examples)

        # unlabeled_features_known_label = convert_examples_to_features(
        #     unlabeled_examples, self.all_label_list, args.max_seq_length, tokenizer)
        # unlabeled_label_known = torch.tensor([f.label_id for f in unlabeled_features_known_label], dtype=torch.long)
        unlabeled_label_known = torch.tensor([f.label_id for f in unlabeled_features], dtype=torch.long)
        ...
        if self.n_known_cls > 0:
            ...
            semi_label_real = torch.cat([labeled_label_ids, unlabeled_label_known])
        else:
            ...
            semi_label_real = unlabeled_label_known

        ind_semi = torch.arange(size_semi, dtype=torch.long)
        self.examples_semi = labeled_examples + unlabeled_examples
        self.text_semi = np.array([e.text_a for e in self.examples_semi])
        return semi_input_ids, semi_input_mask, semi_segment_ids, semi_label_ids, ind_semi, semi_label_real

    def get_semi_loader(self,
                        semi_input_ids, semi_input_mask,
                        semi_segment_ids, semi_label_ids,
                        ind_semi, semi_label_real,
                        args):
    ...
    def get_loader(self, examples, args, mode = 'train'):
        ...
        ind = torch.arange(len(examples), dtype=torch.long)  # added

        data = TensorDataset(input_ids, input_mask, segment_ids, label_ids, ind)
